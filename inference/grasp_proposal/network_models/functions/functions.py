...
from ..nn_utils.functional import bpdist
...
def euler2RotMatrix(euler):
    ...
    cb = torch.cos(b)
    ch = torch.cos(h)
    R = torch.stack([ca * ch, -ch * sa * cb + sh * sb, ch * sa * sb + sh * cb,
                     sa, ca * cb, -ca * sb,
                     -sh * ca, sh * sa * cb + ch * sb, -sh * sa * sb + ch * cb], dim=1).contiguous()
