import open3d
import os
from pcd_classes.torch_precomputed_baseline import TorchDataScenePointCloud, \
    TorchPrecomputedBaselinePointCloud
...
baseline_path = path.get_resource_dir_path('baseline_training_data')


def generate_single_scene(scene_name, num_of_view=4):
    current = time()
    ...
    for i in range(num_of_view):
        if not os.path.exists(single_view_list[i]):
            continue
        view_cloud = TorchPrecomputedBaselinePointCloud(open3d.io.read_point_cloud(single_view_list[i]),
                                                          open3d.io.read_point_cloud(noise_single_view_list[i]),
                                                          camera_pose=camera_pose_quat[i])

        view_cloud.run_score(scene_cloud)

        output_path = os.path.join(baseline_path, "{}_view_{}.p".format(scene_name, i))
